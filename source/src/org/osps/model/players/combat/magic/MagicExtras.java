...
public class MagicExtras {
	...
	public static boolean checkMultiBarrageReqs(Player c, int i) {
		...
		if(Config.COMBAT_LEVEL_DIFFERENCE && !c.isInBarrows() && !c.isInBarrows2()) {
			...
			if(combatDif1 > c.wildLevel && !c.isInBarrows() && !c.isInBarrows2() || combatDif1 > PlayerHandler.players[i].wildLevel && !c.isInBarrows() && !c.isInBarrows2()) {
		...
		if (PlayerHandler.players[playerId] != null) {
			...
			if (c.getCombat().checkMultiBarrageReqs(playerId)) {
				...
				if (Misc.random(c.getCombat().mageAtk()) > Misc.random(c2.getCombat().mageDef()) && !c.magicFailed) {
					...
					c.getPA().addSkillXP((c.MAGIC_SPELLS[c.oldSpellId][7] + damage*1), 6); 
					c.getPA().addSkillXP((c.MAGIC_SPELLS[c.oldSpellId][7] + damage*1 / 1), 3);
