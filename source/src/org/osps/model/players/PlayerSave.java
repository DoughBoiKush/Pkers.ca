...
import org.osps.model.content.presets.PresetContainer;
...
public class PlayerSave {
			...
			if (spot > -1) {
				...
				case 2:
					if (token.equals("character-height")) {
						...
						p.setRights(right);
					} else if (token.equals("screen-brightness")) {
						p.screenBrightness = Integer.parseInt(token2);
					...
					} else if (token.equals("preservePrayer")) {
						p.preservePrayerScroll = Boolean.parseBoolean(token2);
					} else if (token.equals("auguryPrayer")) {
						p.auguryPrayerScroll = Boolean.parseBoolean(token2);
					} else if (token.equals("rigourPrayer")) {
						p.rigourPrayerScroll = Boolean.parseBoolean(token2);
						...
						p.title = token2;
					} else if (token.equals("titles0")){
						String[] g = token2.split(" = ");
						for (int i = 0; i < g.length; i++) {
							p.titleUnlocked[0][i] = Boolean.parseBoolean(g[i]);
						}
					} else if (token.equals("titles1")){
						String[] g = token2.split(" = ");
						for (int i = 0; i < g.length; i++) {
							p.titleUnlocked[1][i] = Boolean.parseBoolean(g[i]);
						}
					} else if (token.equals("titles2")){
						String[] g = token2.split(" = ");
						for (int i = 0; i < g.length; i++) {
							p.titleUnlocked[2][i] = Boolean.parseBoolean(g[i]);
						}
				...
				case 14:
					...
					break;
						...
						p.shop().items[Integer.parseInt(token3[0])] = Integer.parseInt(token3[1]);
	...
	public static boolean saveGame(Player p) {
		...
		try {
			...
			characterfile.newLine();
			characterfile.write("screen-brightness = ", 0, 20);
			characterfile.write(Integer.toString(p.screenBrightness), 0, Integer.toString(p.screenBrightness).length());
			characterfile.newLine();
			...
			characterfile.newLine();
			characterfile.write("preservePrayer = ", 0, 17);
			characterfile.write(Boolean.toString(p.preservePrayerScroll), 0, Boolean.toString(p.preservePrayerScroll).length());
			characterfile.newLine();
			characterfile.write("auguryPrayer = ", 0, 15);
			characterfile.write(Boolean.toString(p.auguryPrayerScroll), 0, Boolean.toString(p.auguryPrayerScroll).length());
			characterfile.newLine();
			characterfile.write("rigourPrayer = ", 0, 15);
			characterfile.write(Boolean.toString(p.rigourPrayerScroll), 0, Boolean.toString(p.rigourPrayerScroll).length());
			characterfile.newLine();
			...
			characterfile.write("character-title = "+p.title);
			characterfile.newLine();
			
			characterfile.write("titles0 = ");
			for (int i = 0; i < p.titleUnlocked[0].length; i++) {
				characterfile.write("" + p.titleUnlocked[0][i] +" = ");
			}
			characterfile.newLine();
			characterfile.write("titles1 = ");
			for (int i = 0; i < p.titleUnlocked[1].length; i++) {
				characterfile.write("" + p.titleUnlocked[1][i] +" = ");
			}
			characterfile.newLine();
			characterfile.write("titles2 = ");
			for (int i = 0; i < p.titleUnlocked[2].length; i++) {
				characterfile.write("" + p.titleUnlocked[2][i] +" = ");
			}
			characterfile.newLine();
			...
			characterfile.newLine();
                        ...
                        characterfile.write("[PLAYER-SHOP]");
			characterfile.newLine();
			for (int i = 0; i < 30; i++) {
				if (p.shop().items[i] > 0) {
					characterfile.write("character-shop = ");
					characterfile.write(Integer.toString(i));
					characterfile.write("	", 0, 1);
					characterfile.write(Integer.toString(p.shop().items[i]));
					characterfile.write("	", 0, 1);
					characterfile.write(Integer.toString(p.shop().amounts[i]));
                                        characterfile.write("	", 0, 1);
					characterfile.write(Integer.toString(p.shop().prices[i]));
					characterfile.newLine();
				}
			}
			characterfile.newLine();
