package org.osps.model.content.teleport;
...
/*		if (player.getCerberusEvent().getInstancedCerberus() != null) {
	 ...
	 */
	public static void teleport(org.osps.model.players.Player player, Teleport teleport, boolean requirements) {
	...
	public static boolean canTeleport(Player player, Teleport tele) {
		...
		//if (!player.startTele2) { TODO
			if (!player.playerName.equalsIgnoreCase("dylan") && player.inWild() && player.wildLevel > Config.NO_TELEPORT_WILD_LEVEL && !player.usingGlory && tele.getType() != Teleport.TeleportType.OBELISK && tele.getType() != Teleport.TeleportType.LEVER) {
				player.sendMessage("You can't teleport above level " + Config.NO_TELEPORT_WILD_LEVEL + " in the wilderness.");
				return false;
			} else if (!player.playerName.equalsIgnoreCase("dylan") && player.usingGlory && player.wildLevel > 30) {
	...
	public static void executeLeverTeleport(Player player, Teleport teleport) {
		if (!player.playerName.equalsIgnoreCase("dylan") && player.inWild() && player.teleBlockLength > 0) {
